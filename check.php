<?php
	...
	foreach($daemons as $daemon => $config){
        ...
        if(file_exists("/etc/init.d/{$daemon}")){
			...
			$running = shell_exec("{$service} {$daemon} status");

			#Start Daemon, if not running, but enabled in configuration file
			if(trim($running) != 'Running' && $config['enabled']) shell_exec("{$service} {$daemon} start");
			
			#Stop Daemon, if running, but disabled in configuration file
			else if(trim($running) == 'Running' && !$config['enabled']) shell_exec("{$service} {$daemon} stop");
